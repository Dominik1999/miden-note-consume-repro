//! Two-client adv.has_mapkey test: Client A creates a custom note that uses
//! `adv.has_mapkey` to check for a Falcon signature before reading it (same
//! pattern as ADN). Client B (separate store/keystore) imports and consumes it
//! with the signature in the advice map.
//!
//! Run with:
//!   RUST_LOG=miden_client::transaction=info cargo test --test two_client_hasmapkey_test -- --ignored --nocapture
...
async fn two_client_hasmapkey() -> anyhow::Result<()> {
    ...
    eprintln!("[hasmapkey] step 1: client A synced");

    // Deploy faucet (Public)
    let faucet_id = deploy_faucet(&mut client_a, &keystore_a, "HMTST").await?;
    eprintln!("[hasmapkey] step 2: faucet deployed (Public): {}", faucet_id.to_hex());
    ...
    eprintln!("[hasmapkey] step 3: wallet A deployed (Public): {}", wallet_a_id.to_hex());
    ...
    let (wallet_b_id, wallet_b_key, wallet_b_bytes) =
        deploy_wallet(&mut client_a, &keystore_a).await?;
    eprintln!("[hasmapkey] step 4: wallet B deployed (Public): {}", wallet_b_id.to_hex());

    client_a.sync_state().await?;
    tokio::time::sleep(Duration::from_secs(2)).await;

    // Mint tokens to wallet A and consume
    eprintln!("[hasmapkey] step 5: minting tokens to wallet A...");
    mint_and_consume(&mut client_a, faucet_id, wallet_a_id, 10_000).await?;
    eprintln!("[hasmapkey] step 5: wallet A funded");
    ...
    eprintln!("[hasmapkey] step 6: note script compiled");
    ...
    let note_id = note.id();
    eprintln!("[hasmapkey] step 6: note built, id={note_id}");

    // Serialize note for client B
    let note_bytes = note.to_bytes();
    eprintln!("[hasmapkey] step 6: note serialized ({} bytes)", note_bytes.len());
    ...
    let create_tx = client_a
        ...
        .await?;
    eprintln!("[hasmapkey] step 7: note submitted on-chain: {create_tx}");

    // Wait for on-chain confirmation
    tokio::time::sleep(Duration::from_secs(5)).await;
    client_a.sync_state().await?;
    eprintln!("[hasmapkey] step 7: client A synced after note submission");
    ...
    eprintln!("[hasmapkey] step 8: client B created and synced");
    ...
    eprintln!("[hasmapkey] step 8: wallet B imported into client B");
    ...
    client_b
        ...
        .await?;
    eprintln!("[hasmapkey] step 9: note imported into client B");
    ...
    for attempt in 0..60 {
        ...
        if !consumable.is_empty() {
            eprintln!(
                "[hasmapkey] step 9: note consumable after {attempt} sync attempts ({} notes)",
                consumable.len()
            );
            break;
        }
        if attempt == 59 {
            eprintln!("[hasmapkey] step 9: WARNING: note never became consumable, trying anyway...");
        }
        tokio::time::sleep(Duration::from_secs(3)).await;
    }
    ...
    match client_b.get_input_note(note_id).await {
        Ok(Some(record)) => {
            eprintln!(
                "[hasmapkey] step 9: note state={:?}, is_authenticated={}",
                record.state(),
                record.is_authenticated()
            );
        }
        Ok(None) => eprintln!("[hasmapkey] step 9: NOTE NOT FOUND in client B"),
        Err(e) => eprintln!("[hasmapkey] step 9: get_input_note error: {e}"),
    }

    // ── Build note_args + Falcon signature for consumption ──
    // Note args: [amount=500, 0, 0, 0]
    ...
    eprintln!("[hasmapkey] step 10: consuming note from client B...");
    eprintln!("[hasmapkey]   note_args={note_args:?}");
    eprintln!("[hasmapkey]   sig_key={sig_key:?}");
    eprintln!("[hasmapkey]   prepared_sig len={}", prepared.len());
        ...
        .await
    {
        Ok(tx_id) => {
            eprintln!("[hasmapkey] ══════════════════════════════════════════════════════");
            eprintln!("[hasmapkey] SUCCESS: hasmapkey note consumed cross-client! tx={tx_id}");
            eprintln!("[hasmapkey] ══════════════════════════════════════════════════════");
            Ok(())
        }
        Err(e) => {
            ...
            eprintln!("[hasmapkey] ══════════════════════════════════════════════════════");
            eprintln!("[hasmapkey] FAILED:");
            eprintln!("[hasmapkey]   {}", &err_str[..err_str.len().min(800)]);
            eprintln!("[hasmapkey] ══════════════════════════════════════════════════════");
            anyhow::bail!("hasmapkey cross-client consume failed: {e}");
        }
    }
}
