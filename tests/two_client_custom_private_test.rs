//! Two-client custom note test (PRIVATE): Client A creates a secret_hash_note with NoteType::Private,
...
//!
//! Run with:
//!   RUST_LOG=miden_client::transaction=info cargo test --test two_client_custom_private_test -- --ignored --nocapture
...
#[tokio::test]
#[ignore]
async fn two_client_custom_note_private() -> anyhow::Result<()> {
    ...
    eprintln!("[custom-priv] client A synced");
    ...
    eprintln!("[custom-priv] step 1: faucet deployed: {}", faucet_id.to_hex());
    ...
    eprintln!("[custom-priv] step 2: wallet A deployed: {}", wallet_a_id.to_hex());
    ...
    eprintln!(
        "[custom-priv] step 3: wallet B created (NOT in client A): {}",
        wallet_b_id.to_hex()
    );

    // 4. Mint to wallet A, consume the mint note
    eprintln!("[custom-priv] step 4: minting tokens to wallet A...");
    ...
    let mint_tx = client_a
        ...
        .await?;
    eprintln!("[custom-priv] step 4: mint tx submitted: {mint_tx}");
    ...
    for attempt in 0..60 {
        ...
        if !consumable.is_empty() {
            eprintln!("[custom-priv] step 4: mint note consumable (attempt {attempt}), consuming...");
            ...
            let consume_tx = client_a
                ...
                .await?;
            eprintln!("[custom-priv] step 4: mint note consumed: {consume_tx}");
            break;
        }
        if attempt == 59 {
    ...
    client_a.sync_state().await?;
    tokio::time::sleep(Duration::from_secs(3)).await;
    eprintln!("[custom-priv] step 4: wallet A funded");

    // 5. Build a custom secret_hash_note MANUALLY with NoteType::Private
    ...
    let note_id = note.id();
    eprintln!("[custom-priv] step 5: custom note built manually (PRIVATE), id={note_id}");
    ...
    let create_tx = client_a
        ...
        .await?;
    eprintln!("[custom-priv] step 6: custom note submitted on-chain: {create_tx}");

    // 7. Serialize note and wallet B
    let note_bytes = note.to_bytes();
    let wallet_b_bytes = wallet_b.to_bytes();
    eprintln!(
        "[custom-priv] step 7: serialized note ({} bytes) and wallet B ({} bytes)",
        note_bytes.len(),
        wallet_b_bytes.len()
    );

    // 8. Wait 10 seconds for on-chain commit
    eprintln!("[custom-priv] step 8: waiting 10s for on-chain commit...");
    ...
    eprintln!("[custom-priv] ══════════════════════════════════════════════════════");
    eprintln!("[custom-priv] CLIENT B: Starting consumer client");
    eprintln!("[custom-priv] ══════════════════════════════════════════════════════");
    ...
    eprintln!(
        "[custom-priv] B step 1: deserialized wallet B={} and note={}",
    ...
    eprintln!("[custom-priv] B step 2: wallet B imported");
    ...
    eprintln!("[custom-priv] B step 3: wallet B key added to keystore");
    ...
    client_b
        ...
        .await?;
    eprintln!("[custom-priv] B step 4: custom note imported with tag");

    // 5. Sync - for private notes, the note will NOT become authenticated
    eprintln!("[custom-priv] B step 5: syncing (note will stay unauthenticated for private notes)...");
    for attempt in 0..20 {
        ...
        if let Ok(Some(record)) = client_b.get_input_note(note_id).await {
            if attempt % 5 == 0 {
                eprintln!(
                    "[custom-priv]   attempt {attempt}: is_authenticated={}",
                    record.is_authenticated()
                );
            }
            if record.is_authenticated() {
                eprintln!("[custom-priv] B step 5: note authenticated (unexpected for private, but OK)!");
                break;
            }
        } else if attempt % 10 == 0 {
            eprintln!("[custom-priv]   attempt {attempt}: note not in store yet");
        }

        tokio::time::sleep(Duration::from_secs(3)).await;
    }
    eprintln!("[custom-priv] B step 5: proceeding with unauthenticated note consumption");

    // 6. Consume via input_notes([(note, Some(secret))]) - works even if unauthenticated
    eprintln!("[custom-priv] B step 6: consuming custom note with secret (unauthenticated)...");
    ...
    match client_b.submit_new_transaction(wallet_b_id, consume_req).await {
        Ok(tx_id) => {
            eprintln!("[custom-priv] ══════════════════════════════════════════════════════");
            eprintln!("[custom-priv] SUCCESS: Custom PRIVATE note consumed cross-client! tx={tx_id}");
            eprintln!("[custom-priv] ══════════════════════════════════════════════════════");
            Ok(())
        }
        Err(e) => {
            ...
            eprintln!("[custom-priv] ══════════════════════════════════════════════════════");
            eprintln!("[custom-priv] FAILED: {}", &err_str[..err_str.len().min(500)]);
            eprintln!("[custom-priv] ══════════════════════════════════════════════════════");
            anyhow::bail!("Custom PRIVATE note cross-client consume failed: {e}");
        }
    }
}
