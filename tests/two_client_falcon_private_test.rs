//! Two-client Falcon-512 signature verification test (PRIVATE).
//!
//! Client A creates a custom note (falcon_p2id_note.masm) with NoteType::Private that requires
//! a Falcon-512 signature to consume. Client B (separate store/keystore) imports the note and
...
//!
//! Run with:
//!   RUST_LOG=miden_client::transaction=info cargo test --test two_client_falcon_private_test -- --ignored --nocapture
...
async fn two_client_falcon_p2id_private() -> anyhow::Result<()> {
    ...
    eprintln!("[falcon-priv] step 1: client A synced");

    // Deploy faucet (Public)
    let faucet_id = deploy_faucet(&mut client_a, &keystore_a, "FTST").await?;
    eprintln!("[falcon-priv] step 2: faucet deployed: {}", faucet_id.to_hex());
    ...
    eprintln!("[falcon-priv] step 3: wallet A deployed: {}", wallet_a_id.to_hex());
    ...
    let (wallet_b_id, wallet_b_key, wallet_b_bytes) =
        deploy_wallet(&mut client_a, &keystore_a).await?;
    eprintln!("[falcon-priv] step 4: wallet B deployed: {}", wallet_b_id.to_hex());

    client_a.sync_state().await?;
    tokio::time::sleep(Duration::from_secs(2)).await;

    // Mint tokens to wallet A and consume
    eprintln!("[falcon-priv] step 5: minting tokens to wallet A...");
    mint_and_consume(&mut client_a, faucet_id, wallet_a_id, 10_000).await?;
    eprintln!("[falcon-priv] step 5: wallet A funded");

    client_a.sync_state().await?;
    tokio::time::sleep(Duration::from_secs(3)).await;

    // ── Create the Falcon P2ID note with NoteType::Private ──
    ...
    eprintln!("[falcon-priv] step 6: Falcon P2ID note script compiled");
    ...
    let note_id = note.id();
    eprintln!("[falcon-priv] step 6: Falcon P2ID note built (PRIVATE), id={note_id}");

    // Serialize note for client B
    let note_bytes = note.to_bytes();
    eprintln!("[falcon-priv] step 6: note serialized ({} bytes)", note_bytes.len());
    ...
    let create_tx = client_a
        ...
        .await?;
    eprintln!("[falcon-priv] step 7: note submitted on-chain: {create_tx}");

    // Wait for on-chain confirmation
    tokio::time::sleep(Duration::from_secs(10)).await;
    client_a.sync_state().await?;
    eprintln!("[falcon-priv] step 7: client A synced after submission");

    // ═══════════════════════════════════════════════════════════════
    // CLIENT B: separate client, import wallet B + note, consume
    // ═══════════════════════════════════════════════════════════════
    eprintln!("[falcon-priv] ══════════════════════════════════════════════════════");
    eprintln!("[falcon-priv] CLIENT B: Starting consumer client");
    eprintln!("[falcon-priv] ══════════════════════════════════════════════════════");

    let tmp_b = tempfile::tempdir()?;
    let (mut client_b, keystore_b) = build_client(tmp_b.path()).await?;
    client_b.sync_state().await?;
    eprintln!("[falcon-priv] step 8: client B created and synced");
    ...
    eprintln!("[falcon-priv] step 8: wallet B imported into client B");
    ...
    client_b
        ...
        .await?;
    eprintln!("[falcon-priv] step 9: note imported into client B");

    // Sync client B - for private notes, the note will NOT become authenticated
    eprintln!("[falcon-priv] step 9: syncing (note will stay unauthenticated for private notes)...");
    for attempt in 0..20 {
        client_b.sync_state().await?;
        if let Ok(Some(record)) = client_b.get_input_note(note_id).await {
            if record.is_authenticated() {
                eprintln!("[falcon-priv] step 9: note authenticated (unexpected for private, but OK) (attempt {attempt})");
                break;
            }
            if attempt % 5 == 0 {
                eprintln!(
                    "[falcon-priv]   attempt {attempt}: is_authenticated={}",
                    record.is_authenticated()
                );
            }
        } else if attempt % 10 == 0 {
            eprintln!("[falcon-priv]   attempt {attempt}: note not in store yet");
        }
        tokio::time::sleep(Duration::from_secs(3)).await;
    }
    eprintln!("[falcon-priv] step 9: proceeding with unauthenticated note consumption");
    ...
    eprintln!("[falcon-priv] step 10: consuming Falcon P2ID note from client B (unauthenticated)...");
    eprintln!("[falcon-priv]   note_args={note_args:?}");
    eprintln!("[falcon-priv]   sig_key={sig_key:?}");
    eprintln!("[falcon-priv]   prepared_sig len={}", prepared.len());
        ...
        .await
    {
        Ok(tx_id) => {
            eprintln!("[falcon-priv] ══════════════════════════════════════════════════════");
            eprintln!("[falcon-priv] SUCCESS: Falcon P2ID PRIVATE note consumed cross-client! tx={tx_id}");
            eprintln!("[falcon-priv] ══════════════════════════════════════════════════════");
            Ok(())
        }
        Err(e) => {
            ...
            eprintln!("[falcon-priv] ══════════════════════════════════════════════════════");
            eprintln!("[falcon-priv] FAILED: {}", &err_str[..err_str.len().min(800)]);
            eprintln!("[falcon-priv] ══════════════════════════════════════════════════════");
            anyhow::bail!("Falcon P2ID PRIVATE cross-client consume failed: {e}");
        }
    }
}
