//! Two-client ADN test: Client A creates a full AgentDebitNote, Client B
//! (separate store/keystore) imports and consumes it with a Falcon signature
//! in the advice map.
//!
//! Run with:
//!   RUST_LOG=miden_client::transaction=info cargo test --test two_client_adn_test -- --ignored --nocapture
...
async fn two_client_adn() -> anyhow::Result<()> {
    ...
    eprintln!("[adn] step 1: client A synced");

    // Deploy faucet
    let faucet_id = deploy_faucet(&mut client_a, &keystore_a, "ADNT").await?;
    eprintln!("[adn] step 2: faucet deployed: {}", faucet_id.to_hex());
    ...
    eprintln!("[adn] step 3: wallet A deployed: {}", wallet_a_id.to_hex());
    ...
    let (wallet_b_id, wallet_b_key, wallet_b_bytes) =
        deploy_wallet(&mut client_a, &keystore_a).await?;
    eprintln!("[adn] step 4: wallet B deployed: {}", wallet_b_id.to_hex());

    client_a.sync_state().await?;
    tokio::time::sleep(Duration::from_secs(2)).await;

    // Mint tokens to wallet A and consume
    eprintln!("[adn] step 5: minting tokens to wallet A...");
    mint_and_consume(&mut client_a, faucet_id, wallet_a_id, 10_000).await?;
    eprintln!("[adn] step 5: wallet A funded");
    ...
    eprintln!("[adn] step 6: ADN note script compiled");
    ...
    let note_id = note.id();
    eprintln!("[adn] step 6: ADN note built, id={note_id}");

    // Serialize note for client B
    let note_bytes = note.to_bytes();
    eprintln!("[adn] step 6: ADN note serialized ({} bytes)", note_bytes.len());
    ...
    let create_tx = client_a
        ...
        .await?;
    eprintln!("[adn] step 7: ADN note submitted on-chain: {create_tx}");

    // Wait for on-chain confirmation
    tokio::time::sleep(Duration::from_secs(10)).await;
    client_a.sync_state().await?;
    eprintln!("[adn] step 7: client A synced after ADN submission");
    ...
    eprintln!("[adn] step 8: wallet B imported into client B");
    ...
    client_b
        ...
        .await?;
    eprintln!("[adn] step 9: ADN note imported into client B");
    ...
    for attempt in 0..60 {
        ...
        if let Ok(Some(record)) = client_b.get_input_note(note_id).await {
            if record.is_authenticated() {
                eprintln!("[adn] step 9: note authenticated after {attempt} syncs");
                break;
            }
        }
        if attempt == 59 {
            eprintln!("[adn] step 9: WARNING: note never authenticated, trying anyway...");
        }
        tokio::time::sleep(Duration::from_secs(3)).await;
    }
    ...
    eprintln!("[adn] step 10: consuming ADN note from client B...");
    eprintln!("[adn]   note_args={note_args:?}");
    eprintln!("[adn]   sig_key={sig_key:?}");
    eprintln!("[adn]   prepared_sig len={}", prepared.len());
        ...
        .await
    {
        Ok(tx_id) => {
            eprintln!("[adn] ══════════════════════════════════════════════════════");
            eprintln!("[adn] SUCCESS: ADN note consumed cross-client! tx={tx_id}");
            eprintln!("[adn] ══════════════════════════════════════════════════════");
            Ok(())
        }
        Err(e) => {
            ...
            eprintln!("[adn] ══════════════════════════════════════════════════════");
            eprintln!("[adn] FAILED: {}", &err_str[..err_str.len().min(800)]);
            eprintln!("[adn] ══════════════════════════════════════════════════════");
            anyhow::bail!("ADN cross-client consume failed: {e}");
        }
    }
}
