//! Two-client P2ID test: Client A creates a P2ID note MANUALLY via `own_output_notes`,
//! then Client B (separate store/keystore) imports and consumes it.
//!
//! Run with:
//!   RUST_LOG=miden_client::transaction=info cargo test --test two_client_p2id_test -- --ignored --nocapture
...
#[tokio::test]
#[ignore]
async fn two_client_p2id() -> anyhow::Result<()> {
    ...
    eprintln!(
        "[p2id] step 3: wallet B created (NOT in client A): {}",
        wallet_b_id.to_hex()
    );

    // 4. Mint to wallet A, consume the mint note
    eprintln!("[p2id] step 4: minting tokens to wallet A...");
    ...
    let mint_tx = client_a
        ...
        .await?;
    eprintln!("[p2id] step 4: mint tx submitted: {mint_tx}");
    ...
    for attempt in 0..60 {
        ...
        if !consumable.is_empty() {
            eprintln!("[p2id] step 4: mint note consumable (attempt {attempt}), consuming...");
            ...
            let consume_tx = client_a
                ...
                .await?;
            eprintln!("[p2id] step 4: mint note consumed: {consume_tx}");
            break;
        }
        if attempt == 59 {
    ...
    client_a.sync_state().await?;
    tokio::time::sleep(Duration::from_secs(3)).await;
    eprintln!("[p2id] step 4: wallet A funded");

    // 5. Build a P2ID note MANUALLY
    ...
    let note_id = note.id();
    eprintln!("[p2id] step 5: P2ID note built manually, id={note_id}");
    ...
    let create_tx = client_a
        ...
        .await?;
    eprintln!("[p2id] step 6: P2ID note submitted on-chain: {create_tx}");

    // 7. Serialize note and wallet B
    let note_bytes = note.to_bytes();
    let wallet_b_bytes = wallet_b.to_bytes();
    eprintln!(
        "[p2id] step 7: serialized note ({} bytes) and wallet B ({} bytes)",
        note_bytes.len(),
        wallet_b_bytes.len()
    );

    // 8. Wait 10 seconds for on-chain commit
    eprintln!("[p2id] step 8: waiting 10s for on-chain commit...");
    ...
    eprintln!("[p2id] ══════════════════════════════════════════════════════");
    eprintln!("[p2id] CLIENT B: Starting consumer client");
    eprintln!("[p2id] ══════════════════════════════════════════════════════");
    ...
    eprintln!(
        "[p2id] B step 1: deserialized wallet B={} and note={}",
    ...
    eprintln!("[p2id] B step 2: wallet B imported");
    ...
    eprintln!("[p2id] B step 3: wallet B key added to keystore");
    ...
    client_b
        ...
        .await?;
    eprintln!("[p2id] B step 4: P2ID note imported with tag");

    // 5. Sync, check note state
    eprintln!("[p2id] B step 5: syncing to authenticate note...");
    for attempt in 0..60 {
        ...
        if let Ok(Some(record)) = client_b.get_input_note(note_id).await {
            if attempt % 5 == 0 || record.is_authenticated() {
                eprintln!(
                    "[p2id]   attempt {attempt}: is_authenticated={}",
                    record.is_authenticated()
                );
            }
            if record.is_authenticated() {
                eprintln!("[p2id] B step 5: note authenticated!");
                break;
            }
        } else if attempt % 10 == 0 {
            eprintln!("[p2id]   attempt {attempt}: note not in store yet");
        }

        if attempt == 59 {
            eprintln!("[p2id] WARNING: note never became authenticated after 60 attempts");
        }
        tokio::time::sleep(Duration::from_secs(3)).await;
    }

    // 6. Consume via input_notes([(note, None)])
    eprintln!("[p2id] B step 6: consuming P2ID note...");
    ...
    match client_b.submit_new_transaction(wallet_b_id, consume_req).await {
        Ok(tx_id) => {
            eprintln!("[p2id] ══════════════════════════════════════════════════════");
            eprintln!("[p2id] SUCCESS: P2ID note consumed cross-client! tx={tx_id}");
            eprintln!("[p2id] ══════════════════════════════════════════════════════");
            Ok(())
        }
        Err(e) => {
            ...
            eprintln!("[p2id] ══════════════════════════════════════════════════════");
            eprintln!("[p2id] FAILED: {}", &err_str[..err_str.len().min(500)]);
            eprintln!("[p2id] ══════════════════════════════════════════════════════");
